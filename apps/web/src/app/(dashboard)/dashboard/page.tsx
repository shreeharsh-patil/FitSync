...
import { Activity, Flame, Trophy, TrendingUp, Calendar, ArrowRight, Dumbbell, Utensils, Zap, Search, Brain } from "lucide-react"
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
        <div className="space-y-2">
          <h1 className="text-5xl font-bold font-heading tracking-tight leading-tight">
            Good morning, <span className="text-secondary">Alex</span>
          </h1>
          <p className="text-muted-foreground text-lg">Your fitness ecosystem is synchronized and ready.</p>
        </div>
        
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
          <div className="relative w-full sm:w-64">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search data..." className="pl-10 h-12 bg-card/50 border-white/5 rounded-2xl" />
          </div>
          <div className="flex items-center gap-4 bg-secondary/10 px-6 py-3 rounded-2xl border border-secondary/20 shadow-lg shadow-secondary/5">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-secondary text-lg leading-none">12 Days</span>
              <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Current Streak</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 p-10 glass border-white/5 rounded-[3rem] space-y-10 relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold font-heading">Performance Analytics</h2>
              <p className="text-sm text-muted-foreground">Volume and frequency over the last 30 days.</p>
            </div>
            <div className="flex bg-muted/50 p-1 rounded-xl border border-white/5">
              <Button variant="ghost" size="sm" className="text-xs h-8 px-4 rounded-lg bg-background shadow-sm font-bold">Week</Button>
              <Button variant="ghost" size="sm" className="text-xs h-8 px-4 rounded-lg text-muted-foreground hover:text-foreground font-bold">Month</Button>
              <Button variant="ghost" size="sm" className="text-xs h-8 px-4 rounded-lg text-muted-foreground hover:text-foreground font-bold">Year</Button>
            </div>
          </div>
          ...
          <div className="h-[350px] w-full bg-background/30 rounded-[2rem] border border-white/5 flex flex-col items-center justify-center relative overflow-hidden group/chart">
            <div className="absolute inset-x-12 bottom-0 flex items-end justify-around h-full opacity-30 group-hover/chart:opacity-50 transition-opacity">
               {[40, 70, 45, 90, 65, 80, 50, 85, 60, 95].map((h, i) => (
                 <div key={i} className="w-8 bg-gradient-to-t from-secondary/80 to-secondary rounded-t-xl transition-all duration-700 delay-[i*50ms]" style={{ height: `${h}%` }}>
            ...
            <div className="relative z-10 flex flex-col items-center gap-4 text-center p-8 bg-background/50 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl">
              ...
              <p className="text-sm text-muted-foreground max-w-xs">Your training volume is peaking. Consider a deload week to optimize recovery.</p>
              <Button variant="link" className="text-secondary font-bold p-0 flex items-center gap-2 group-hover:translate-x-1 transition-transform">
                View Full Report
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            <h2 className="text-xl font-bold font-heading px-2">Action Center</h2>
               ...
               <div className="space-y-4">
                  <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-secondary/20 transition-all cursor-pointer group">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Starts in 2 days</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-accent/20 transition-all cursor-pointer group">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Verify Bench Press</p>
                    </div>
                  </div>
               </div>
          ...
          <Card className="p-8 bg-gradient-to-br from-accent/20 to-primary/20 border border-white/5 rounded-[2.5rem] relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-4">
               ...
               <p className="text-sm text-muted-foreground leading-relaxed">
                 You are 15% more likely to hit your targets when you train before 10 AM. Your sleep data suggests tomorrow is a peak day.
               </p>
               <Button variant="link" className="p-0 h-auto text-accent font-bold uppercase tracking-widest text-[10px] hover:text-accent/80 transition-colors">
...
function SummaryCard({ icon, label, value, subtext, gradient }: { icon: React.ReactNode, label: string, value: string, subtext: string, gradient: string }) {
  return (
    <Card className="p-8 glass border-white/5 rounded-[2.5rem] hover:border-white/10 transition-all group relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-1">
          <p className="text-2xl font-bold font-heading group-hover:text-secondary transition-colors tracking-tight">{value}</p>
