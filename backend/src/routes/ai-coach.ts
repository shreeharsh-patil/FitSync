...
router.post("/", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const recentWorkouts = await Workout.find({ userId: req.user!.userId }).sort({ logDate: -1 }).limit(3).lean();

    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const todayMeals = await Nutrition.find({ userId: req.user!.userId, logDate: { $gte: today } }).lean();
