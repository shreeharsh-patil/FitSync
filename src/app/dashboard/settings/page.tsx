...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {successMsg && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl text-sm font-semibold flex items-center gap-2 animate-fade-in">
          ...
          {sections.map((s) => (
            <button key={s.id} onClick={() => setActiveSection(s.id)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                activeSection === s.id ? "bg-accent-coral/10 text-accent-coral" : "text-text-muted hover:text-text-primary hover:bg-bg-secondary"
              }`}>
        ...
        <div className="lg:col-span-3 space-y-6">
          {activeSection === "profile" && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              <div className="rounded-2xl bg-bg-card border border-border p-6 md:p-8 space-y-6">
                <h2 className="text-lg font-bold font-heading text-text-primary">Profile Information</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <label htmlFor="name" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Full Name</label>
                    <input id="name" name="name" value={profile.name} onChange={(e) => updateProfile("name", e.target.value)} className="input w-full" />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="email" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Email</label>
                    <input id="email" name="email" value={profile.email} disabled className="input w-full opacity-50 cursor-not-allowed" />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="fitnessGoal" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Fitness Goal</label>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="activityLevel" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Activity Level</label>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="height" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Height (cm)</label>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="weight" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Weight (kg)</label>
                ...
                <div className="space-y-2">
                  <label htmlFor="bio" className="text-[9px] font-semibold uppercase tracking-[0.12em] text-text-muted">Bio</label>
                ...
                <div className="flex justify-end gap-4 pt-4 border-t border-border">
                  <button onClick={() => initialProfile && setProfile({ ...initialProfile })}
                    className="px-5 py-2.5 border border-border rounded-xl text-xs font-semibold text-text-muted hover:text-text-primary transition-all">Cancel</button>
              ...
              <div className="rounded-2xl bg-bg-card border border-border p-6 md:p-8 space-y-4">
                ...
                {[
                  ...
                  <div key={key} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                    ...
                    <button
                      ...
                      className={`text-[9px] px-2.5 py-1 rounded-lg font-semibold uppercase tracking-wider transition-all ${
                        integrations[key]
                          ? "bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-red-50 hover:text-red-700 hover:border-red-200"
                          : "bg-bg-secondary text-text-muted border border-border hover:bg-accent-coral/10 hover:text-accent-coral hover:border-accent-coral/20"
                      }`}>
                ...
                return (
                  <div key={key} className="flex items-center justify-between py-4 border-b border-border last:border-0">
                    ...
                    <button onClick={() => toggleNotification(key)}
                      className={`w-12 h-6 rounded-full transition-all relative ${value ? "bg-accent-coral" : "bg-bg-secondary"}`}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between py-4 border-b border-border">
                  ...
                  <button onClick={() => updateProfile("isPublic", !profile.isPublic)}
                    className={`w-12 h-6 rounded-full transition-all relative ${profile.isPublic ? "bg-accent-coral" : "bg-bg-secondary"}`}>
          ...
          {activeSection === "billing" && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="p-5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-emerald-700">Premium Plan</p>
                  <p className="text-sm text-text-muted">$9.99 / month · Active</p>
                </div>
                <button className="px-4 py-2 bg-accent-coral text-white font-bold text-xs rounded-xl">Manage</button>
              </div>
              <div className="space-y-4 pt-4">
                <h3 className="font-semibold text-sm text-text-primary">Payment Methods</h3>
                <div className="flex items-center gap-4 p-4 rounded-xl bg-bg-secondary border border-border">
